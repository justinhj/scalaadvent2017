...
Here, you encounter a pair of dueling generators. The generators, called generator A and generator B, are trying to agree
on a sequence of numbers. However, one of them is malfunctioning, and so the sequences don't always match.
...
object Day15 {
  ...
  // Make a generator
  def makeGen(factor: Long) : Long => Long = {
  ...
  def countMatches(aGen : Long => Long, bGen : Long => Long, initialA : Long, initialB : Long, iterations: Long) : Long = {

    val sampleResult = (1L to iterations).foldLeft((aGen(initialA), bGen(initialB), 0)) {
      case ((nextA, nextB, matches), _) =>

        val inc = if (nextA % 65536 == nextB % 65536) 1 else 0

        (aGen(nextA), bGen(nextB), matches + inc)
    }
  ...
  def main(args: Array[String]) : Unit = {
    ...
    val sampleGenA = makeGen(16807)
    val sampleGenB = makeGen(48271)

    assert(countMatches(sampleGenA, sampleGenB, 65, 8921, 5) == 1)

    // Sample with 40m

    assert(countMatches(sampleGenA, sampleGenB, 65, 8921, 40000000) == 588)

    // Step 1

    val step1Matches = countMatches(sampleGenA, sampleGenB, 512, 191, 40000000)

    println(s"step1Matches $step1Matches")

    // For part two the generators are similar but only output when the digit is a multiple of some number
    // In fact we can keep the original generators and arrange for them to be called until the output passes the test

    def moduloGenerator(gen: Long => Long, multiple: Long): Long => Long = {

      n =>

        var nextVal = gen(n)

        while (nextVal % multiple != 0) {
          nextVal = gen(nextVal)
        }
    ...
    val step2SampleGenA = moduloGenerator(sampleGenA, 4)
    val step2SampleGenB = moduloGenerator(sampleGenB, 8)

    val step2Matches = countMatches(step2SampleGenA, step2SampleGenB, 512, 191, 5000000)

    println(s"step2Matches $step2Matches")
