...
object Day7 {
  ...
  // A map of which program is supported by which other program
  // We can then pick any node and follow it down to the bottom
  def builtSupportedByMap(programs: Map[String, Program]) : Map[String, String] = {

     programs.foldLeft(Map.empty[String, String]) {

       case (supportedByMap, (supporter, supporting)) =>

         val supported = supporting.supporting.map{s => (s, supporter)}

         supportedByMap ++ supported
     }
  }

  def getBottom(supportNetwork : Map[String, String]) : String = {

    def helper(supportNetwork :  Map[String, String], current: String) : String = {

      supportNetwork.get(current) match {
        case Some(supporter) =>
          helper(supportNetwork, supporter)
        case None =>
          current
      }
    ...
    helper(supportNetwork, supportNetwork.keys.head)

  }

  case class SupportingProgram(name: String, weight: Int, supporting: List[SupportingProgram])

  def buildTree(programs : Map[String, Program], supportedBy : Map[String,String], root: String) : SupportingProgram = {

    val program = programs.get(root)

    val supporting = supportedBy.filter {
      case (supportedBy,supporter) => supporter == root
    }.keys

    val supportingPrograms = supporting.map {
      thisRoot =>
        buildTree(programs, supportedBy, thisRoot)
    }.toList
  ...
  def main(args : Array[String]) : Unit = {
    ...
    val samplePrograms = programsFromInput(lines)

    // Now we have the programs in a format that can easily be used
    // lets build the support network

    val supportNetwork = builtSupportedByMap(samplePrograms)

    val bottom = getBottom(supportNetwork)

    assert(bottom == "tknk")

    // Next solve step 1 input

    val step1InputLines = step1Input.split('\n').toList

    val step1Programs = programsFromInput(step1InputLines)
    val step1SupportNetwork = builtSupportedByMap(step1Programs)
    val step1Bottom = getBottom(step1SupportNetwork)

    println(s"Step 1 supporter is $step1Bottom")

    // For step 2 we want to actually build the tree from the root
