...
object Day2 {
  ...
  val testInput = """1364	461	1438	1456	818	999	105	1065	314	99	1353	148	837	590	404	123
                     ...
                     |4463	4757	201	186	3812	2413	2085	4685	5294	5755	2898	200	5536	5226	1028	180""".stripMargin
  ...
  import fs2.{Stream, text, Pure}

  def toListOfInts(s: String): List[Int] = {
    s.split("\\s+").map {
      _.toInt
    }.toList
  }
  def checkSum(str: String) : Int = {

    def checkSumLine(nums: List[Int]): Int = {
      val min = nums.min
      val max = nums.max
      max - min
    }
  ...
  // Find one set of divisible numbers in the list
  def combs(n: List[Int]) : Int = {
    (for(
      i <- n;
      m <- n
      if (i % m) == 0 && (i/m) > 1
  ...
  def main(args : Array[String]): Unit = {
    ...
    assert(checkSum(sampleInput) == 18)

    println(checkSum(testInput))

    assert(findDivisible("""5 9 2 8
                           |9 4 7 3
                           |3 8 6 5""".stripMargin) == 9)


    println(findDivisible(testInput))
