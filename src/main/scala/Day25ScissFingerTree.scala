...
// Run a program on a turing machine, using finger tree implenentation from
// https://github.com/Sciss/FingerTree

object Day25ScissFingerTree {
  ...
  case class TuringMachine(currentSlot: Int, slots : IndexedSeq[Int], currentState: Char, program: Program, steps : Int = 0) {
...
//      val updatedSlots = slots(currentSlot) = op.writeValue
//
//      op.move match {
//
//        case Left =>
...
//      }
    ...
    def runAllStepsGetCount() : Int = {
      ...
      val what = finalMachine.slots(n => n == 1)
      what.length
    }
  ...
  def main(args : Array[String]) : Unit = {
    ...
    val sample1 = TuringMachine(0, Vector(0), 'a', Program('a', 6, sampleStates))
    ...
    val step1TuringMachine = TuringMachine(0, Vector(0), step1.get.startState, step1.get)
