...
// Run a program on a turing machine, using Cats Chain for the turing tape

object Day25CatsChain {
  ...
  case class TuringMachine(currentSlot: Int, slots : Chain[Int], currentState: Char, program: Program, steps : Int = 0) {
    ...
    def executeOp(op: Op) : TuringMachine = {
      ...
      // TEMP convert to vector and back to update a slot

      val vec = slots.toVector

      val updatedSlotsList: Seq[Int] = vec.updated(currentSlot, op.writeValue).toList

      val updatedSlots: Chain[Int] = Chain.fromSeq(updatedSlotsList)
    ...
    def step() : TuringMachine = {
      ...
      // TEMP convert to vector so we can access the current slot

      val vec = slots.toVector

      // Execute the states op depending on if it is zero or one at current tape slot
      val currentValue = vec(currentSlot)
  ...
  def main(args : Array[String]) : Unit = {
    ...
    val sample1 = TuringMachine(0, Chain(0), 'a', Program('a', 6, sampleStates))
    ...
    val step1TuringMachine = TuringMachine(0, Chain(0), step1.get.startState, step1.get)
